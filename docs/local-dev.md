...
> Flyway가 backend 첫 부팅 시 전체 마이그레이션(2026-07-02 기준 V1~V25)을 자동 적용하므로 schema/seed를 미리 만들 필요 없다.
...
2026-MM-DD HH:MM:SS  INFO ... Flyway ... Successfully applied 25 migrations
```

전체 마이그레이션 적용은 빈 DB에서 첫 부팅 1회만. 두 번째부터는 "no migration necessary". (신규 마이그레이션이 추가되면 개수는 늘어난다 — `backend/src/main/resources/db/migration`이 진실.)
...
### 6.2 CSRF 토큰 헤더 이름 (구: `/api/auth/login` 빈 body 401 — **해결됨**)

**상태 (2026-07-02 closure)**: T2 fix 머지 완료. `config/CsrfAwareAccessDeniedHandler.java`가 CSRF 실패를 `403 + {"code":"CSRF_MISMATCH"}`로 직접 응답해(`SecurityConfig` 배선 포함) 종전의 "빈 body 401" 위장 흐름은 더 이상 발생하지 않는다 — 이제 CSRF 실패는 명확한 403으로 표면화된다.

**여전히 유효한 주의**: 헤더 이름은 Spring 기본값(`X-XSRF-TOKEN`)이 아니라 **`X-CSRF-Token`** 이다 (`SecurityConfig` 명시 설정). curl로 직접 호출 시 §5.1 예시대로 이 이름을 사용할 것. frontend는 `frontend/src/lib/csrf.ts`가 동일 이름을 쓰므로 정상.
...
- `curl http://localhost:8080/actuator/health` — `{"status":"UP"}`이면 backend+DB 정상, `DOWN`(503)이면 DB 연결 문제 (ADR #50)
