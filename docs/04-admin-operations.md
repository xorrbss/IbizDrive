...
3. **Phase 3 (완료, 본 PR 2026-05-12)** — backend: `AdminUserQuotaService` + `GET/PUT /api/admin/users/{id}/quota` (admin only) + `AuditEventType.ADMIN_QUOTA_CHANGED` (audit value `admin.quota.changed`, `docs/03 §4.1`) + `UserQuotaAuditListener` AFTER_COMMIT. `User` entity 매핑 + `changeStorageQuota` invariant 가드. idempotent (same-value no-op, audit emit 생략). 한도 < 사용량(over-quota) 허용 — Phase 5에서 신규 업로드만 차단. service Mockito 단위 테스트 8건 + listener 단위 2건.
