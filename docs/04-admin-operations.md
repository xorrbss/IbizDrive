...
5. **Phase 5 (완료, 본 PR 2026-05-12)** — enforcement: `FileUploadService.upload` (신규 파일 + 새 버전 append 모두 동일 경로) 진입에서 `UserQuotaEnforcer.consumeOrThrow` 호출 — `UserRepository.lockActiveById` (`@Lock PESSIMISTIC_WRITE`)로 행 잠금 + `storage_used + sizeBytes > storage_quota` 가드 + 통과 시 `storage_used` 증분 (CLAUDE.md §3 원칙 7 `FOR UPDATE`). 한도 초과 시 `QuotaExceededException` → `GlobalExceptionHandler`가 HTTP **413 PAYLOAD_TOO_LARGE** + envelope `QUOTA_EXCEEDED` (details: currentUsed/quota/requestedDelta) 매핑 (docs/02 §8 line 2577 + endpoint specs 1360/1365/1415/1472/1495 정합). storage write 이전에 가드 호출 → 객체 orphan 차단. 한도 < 사용량(over-quota — admin 한도 축소)도 신규만 차단, 기존 파일 보존. `FileVersionMutationService.restoreVersion` 등 storage 신규 객체 미생성 경로는 본 트랙 scope 밖 (별도 follow-up 가능). **storage_used 감소(휴지통 영구 삭제, hard delete cron)는 Phase 6** — monotonically 증가만 본 트랙. enforcer Mockito 단위 8건 + FileUploadService Testcontainers slice 4건 (over-quota 차단 / 정확히 한도 경계 / 정상 증분 / 0 byte).
