...
3. **Phase 3 (완료, #198 2026-05-12)** — backend: `AdminUserQuotaService` + `GET/PUT /api/admin/users/{id}/quota` (admin only) + `AuditEventType.ADMIN_QUOTA_CHANGED` (audit value `admin.quota.changed`, `docs/03 §4.1`) + `UserQuotaAuditListener` AFTER_COMMIT. `User` entity 매핑 + `changeStorageQuota` invariant 가드. idempotent (same-value no-op, audit emit 생략). 한도 < 사용량(over-quota) 허용 — Phase 5에서 신규 업로드만 차단. service Mockito 단위 테스트 8건 + listener 단위 2건.
4. **Phase 4 (완료, #203 2026-05-12)** — frontend: `/admin/members` 페이지에 quota 컬럼 추가. `AdminUserQuotaCell` 인라인 editor (GB 단위 정수 입력, `formatBytes`로 used/quota + 비율 표시, over-quota 시 신규 차단 인라인 경고). `AdminUserSummaryResponse`에 `storageQuota`/`storageUsed` 필드 추가 → 행 단위 추가 fetch 회피. `useAdminUpdateUserQuota` hook + `api.adminUpdateUserQuota` mutation + admin users prefix 무효화. user-self quota indicator(`api.getStorageQuota` mock + `StorageBar`)는 별도 `/api/me/storage` endpoint 신설 필요 → 본 PR scope 밖.
5. **Phase 5 (완료, #204 2026-05-12)** — enforcement: `FileUploadService.upload` (신규 파일 + 새 버전 append 모두 동일 경로) 진입에서 `UserQuotaEnforcer.consumeOrThrow` 호출 — `UserRepository.lockActiveById` (`@Lock PESSIMISTIC_WRITE`)로 행 잠금 + `storage_used + sizeBytes > storage_quota` 가드 + 통과 시 `storage_used` 증분 (CLAUDE.md §3 원칙 7 `FOR UPDATE`). 한도 초과 시 `QuotaExceededException` → `GlobalExceptionHandler`가 HTTP **413 PAYLOAD_TOO_LARGE** + envelope `QUOTA_EXCEEDED` (details: currentUsed/quota/requestedDelta) 매핑 (docs/02 §8 line 2577 + endpoint specs 1360/1365/1415/1472/1495 정합). storage write 이전에 가드 호출 → 객체 orphan 차단. 한도 < 사용량(over-quota — admin 한도 축소)도 신규만 차단, 기존 파일 보존. `FileVersionMutationService.restoreVersion` 등 storage 신규 객체 미생성 경로는 본 트랙 scope 밖 (별도 follow-up 가능). enforcer Mockito 단위 8건 + FileUploadService Testcontainers slice 4건 (over-quota 차단 / 정확히 한도 경계 / 정상 증분 / 0 byte).
6. **Phase 6 (완료, 본 PR 2026-05-13)** — hard delete decrement: `User.releaseStorage(delta)` (음수 clamp + invariant `delta >= 0`) + `UserQuotaEnforcer.release(userId, delta)` (`lockActiveById` → graceful no-op if soft-deleted user). `FileRepository.sumVersionBytesPerOwnerByFileIds`로 hard delete 직전 owner별 file_versions sizeBytes 합산, DELETE 후 owner별 1회 release 호출. 적용 site 3개: `TrashPurgeService.purgeFile` (단건) / `TrashPurgeService.purgeFolder` (cascade, 후손 file 일괄 집계) / `HardPurgeService` (`purge.expired` 배치 cron). storage_used 음수 가드: Phase 5 도입 이전 업로드된 파일 release 시 발생 가능 → 0으로 clamp + warn 로그. enforcer release 단위 6건 추가 (userId null / delta 음수 / delta=0 lock-skip / user missing graceful / normal / clamp). monotonic 한계 해소 — `storage_used`가 실제 disk 점유량과 수렴.
