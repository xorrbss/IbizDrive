...
| 13 | **업로드 = tus-java-server (S3 multipart 위임)** | ADR #7 superseded. 표준 프로토콜 재개·체크섬 검증을 자체 구현 회피, S3 multipart는 라이브러리가 위임 처리. M5.1 useUpload 인터페이스 안정. **Status: Superseded by ADR #36 (2026-05-02)** — A15 트랙에서 단일-POST multipart MVP로 재정정 (ADR #7 복귀) + S3 추상화는 별도 `StorageClient`로 도입. tus는 v1.x 재개 백업 트랙. | 01 §9, 02 §6.1 |
...
| 19 | **비밀번호 해싱·정책 = BCrypt(strength=12) + 최소 12자 (영·숫 각 1자, 공백 금지)** | Spring Security 기본 `BCryptPasswordEncoder`, BouncyCastle 의존성 회피. Argon2id 마이그레이션은 `DelegatingPasswordEncoder`로 추후 가능. 길이 우선 정책(OWASP), 특수문자 강제는 사용자 저항. zxcvbn/HIBP 사전 공격 방지는 v1.x. **Closure (auth-password-policy, 2026-05-04)** — ADR #41이 self-signup MVP 진입 마찰 회피 명목으로 임시 완화한 min=8 규정을 **본문(min 12 + 영·숫 + 공백 금지)으로 회복**. 공통 validator(`auth.validation.PasswordPolicyValidator` + `@ValidPassword`) 신설, `SignupRequest`/`ResetPasswordRequest`/`ChangePasswordRequest` 3 DTO 적용, `AuthExceptionHandler.ruleOf`가 ValidPassword violation을 5규칙(`min_length`/`max_length`/`missing_alpha`/`missing_digit`/`whitespace`) rule code로 노출. Frontend 미러(`lib/password.ts`) + 3 페이지(`/signup`·`/reset-password`·`/account/password`)에 동일 매트릭스 사전검증 + 한국어 rule 메시지. `TempPasswordGenerator`는 영문 1자 + 숫자 1자 강제 주입 + Fisher-Yates shuffle로 정책 회귀 가드. 핵심 원칙 11(FE/BE identical logic) 준수 — 양쪽 ASCII letter/digit + Unicode whitespace(NBSP 포함)로 정렬. | 03 §2.7 |
...
| 21 | **사용자 등록 = 관리자 초대 only (셀프 가입 금지)** | 사내 시스템 → 셀프 가입 부적절. MVP는 (a) Flyway 시드로 admin 1명 + (b) `POST /api/admin/users` (admin이 email/임시PW 지정)로 user 생성. 첫 로그인 시 PW 변경 강제. 초대 메일은 A1.5(이메일 인프라 도입 시). **Status: Superseded by ADR #41 (2026-05-02)** — auth-pages 트랙에서 셀프 가입 + first-user-ADMIN 부트스트랩으로 정정. 초기 admin 시드 의존성 제거(빈 DB 첫 가입자가 ADMIN). **잔여 closure (m-admin-entry-rewrite, 2026-05-03)** — 운영자 초대(`POST /api/admin/users`) 활성화 + `/admin` shell(가드 분리, 사이드 nav, landing) + `/admin/users` 초대 폼 + `admin.user.created` audit emit 활성. 임시 PW 비노출 4채널(응답/로그/audit/예외) 보장. 강제 변경 UX는 §2.7 auth-must-change-pw에서 이미 활성. self-signup과 admin invite 두 경로 모두 운영. 별도 ADR 신설 없이 본 ADR 본문에 closure 기록. Prod SMTP 도입은 v1.x. | 03 §2.7, §2.8, §2.10, 02 §7.12, 04 §1, §2 |
...
| 36 | **A15 storage 모듈 + 업로드/다운로드 = `StorageClient` 추상화 + `LocalFsStorageClient` MVP impl + 단일-POST multipart 업로드 + GET `/{id}/download` (RFC 5987 + ETag(versionId)). ADR #13 supersede(tus→multipart). 권한 = upload `UPLOAD` / download `READ`(별도 `DOWNLOAD` enum 미도입). audit emission = file/ 패키지 직접 호출 convention(`emitAudit`) 답습. storage orphan은 MVP 한정 알려진 한계.** | **MVP = 단일-POST multipart**: tus 프로토콜 v1.x 재이월 — KISS+YAGNI. tus 도입 비용(클라이언트 라이브러리 + S3 multipart 라이브러리 위임 + 재개 토큰 만료 정책 + tus-java-server lifecycle) 대비 MVP 파일 분포(< 100MB 다수)에서 ROI 역전. `app.storage.{type, root}` properties + `spring.servlet.multipart.{max-file-size, max-request-size}` 100MB cap. **Storage 추상화** = interface `StorageClient { write(key, in, size); InputStream read(key); delete(key); }` (UUID `storage_key` 식별 — ADR #5 정합) + `LocalFsStorageClient` ({YYYY}/{MM}/{UUID} 객체 키). S3 impl은 v1.x — AWS SDK v2 의존성 미추가. **권한** = upload `hasPermission(#req.folderId, 'folder', 'UPLOAD')` / download `hasPermission(#id, 'file', 'READ')`. 별도 `DOWNLOAD` enum 도입 안 함 — `READ`가 view+download 모두 grant (KISS, docs/03 §3 매트릭스 단순화). **audit emission convention** = file/ 패키지 기존 `emitAudit` helper 직접 호출 (FileMutationService:298-315 패턴). share/permission/auth는 listener 패턴이지만 file/ 패키지 내부 일관성을 위해 직접 호출 채택 — 동일 패키지에 두 패턴 혼재 회피 (KISS+§3). FILE_UPLOADED/FILE_DOWNLOADED 이벤트 활성화. **storage orphan**: 트랜잭션 실패 시 storage 객체 잔존 가능 — MVP 한정 알려진 한계, cleanup job은 별도 트랙. **frontend** = `api.uploadFile`이 실 `XMLHttpRequest`로 `POST /api/files` (multipart, `withCredentials=true`); 409 envelope `{error:{code:'RENAME_CONFLICT', details?:{fileId,fileName}}}` 파싱 (`UploadConflictDialog`는 `task.file.name` 폴백). FakeXHR 모듈 삭제. **Conflict resolution wire format** = `new_version` | `rename` | unset(409). M5 frontend 인터페이스 1:1. **자동 RENAME suffix** = `(N)`. **응답 status** = 신규 파일 INSERT → 201 Created, NEW_VERSION 분기 → 200 OK (`UploadResponse{file, versionId, versionNumber, newFile}`). **Download 응답 헤더** = `Content-Type`(version.mimeType, null/invalid → octet-stream) + `Content-Length`(version.sizeBytes) + `ETag("<versionId>")` + `Content-Disposition: attachment; filename="<ascii-fallback>"; filename*=UTF-8''<percent-encoded>` (RFC 5987 — UTF-8 + 비ASCII 안전). | 02 §6.1/§7.6/§7.7 (supersede), A15 (`a15-file-upload-download`) |
| 37 | **A16 department subject picker = department 도메인 도입(V7) + `GET /api/departments/search` + ShareDto.subjectName 필드 + `findEffective` SQL dept 매칭 분기 + frontend subjectType 3-way picker(everyone\|user\|department). Role share UI 보류(schema impedance, backlog).** | **Department 도메인 도입** — A14 결정 시점("department V_ 마이그레이션 0 → 도메인 부재") 제약 해소. V7 마이그레이션이 `departments` 테이블(id UUID, name CITEXT NOT NULL, path LTREE, active BOOLEAN, timestamps) + `users.department_id UUID NULL FK`를 추가. **LTREE 컬럼은 schema 도입만, 애플리케이션은 flat 사용** (계층 트리 v1.x deferred — KISS). **검색 endpoint 1:1 답습** — A14 `UserSearchController`/`Service`/`Repository` 패턴 동형. `GET /api/departments/search?q=&limit=` (`isAuthenticated()` 공개, q minLength 2 normalize 후, limit default 20/cap 50, `INVALID_SEARCH_QUERY` reuse, LIKE escape, cursor 미지원). audit emission 없음(A14 일관). **권한 평가 SQL 변경 필수** — A14 결정 #4 검증 결과 정정: department subject 도입 시 `PermissionRepository.findEffective`가 user grant + role grant + everyone literal에 더해 **dept 매칭 subquery**(`p.subject_id = (SELECT department_id FROM users WHERE id=:userId AND active)`)를 OR 분기로 추가. `users.department_id` NULL이면 dept 매칭 unmatched(false). **ShareDto.subjectName surface = backend join** (A13 패턴 1:1 답습) — ShareDto에 nullable `subjectName: String|null` 필드 추가. ShareCommandService(단건 helper, 트랜잭션 내 N=subjects.size lookup) + ShareQueryService(batch helper, 페이지 당 type별 1회 IN 절)로 resolve. user → `users.display_name`, department → `departments.name`, everyone/lookup miss → null fallback. 단건 lookup endpoint 미추가(N+1 회피 + endpoint 표면적 ↓). **Role share UI 보류 (결정 #5)** — `subject_type='role'`은 V5 schema에 정의되어 있고 backend `findEffective`가 처리하지만, frontend share dialog의 picker 라디오는 3종(everyone/user/department)으로 한정. role schema impedance(role enum vs role-grant lookup): role 4종(ADMIN/MEMBER/AUDITOR) 노출은 시스템 권한 대비 share scope 의미 불명확. v1.x role-share 트랙으로 분리 (별도 ADR). **Frontend = F6 답습 1:1** — `useDepartmentSearch`(useUserSearch 동형: debounce 300ms / minLen 2 / keepPreviousData / signal / staleTime 30s) + `DepartmentSearchCombobox`(UserSearchCombobox 동형, 표시 필드만 `name` 단일) + ShareDialog `subjectType` 3-way 라디오 + `subjectLabel(type, id, subjectName)` subjectName-first fallback. Combobox 일반화 거부 (KISS, 추상화 정당화 3+ 규칙 — ADR #28 동형). **`subjectType` 4 wire 그대로 보존** — backend는 `everyone`/`user`/`department`/`role` 4종 모두 persistable, 프론트만 3종 노출. v1.x role UI 활성화 시 frontend만 변경. | 00 §5, 02 §2.x/§7.9/§7.15, 03 §3, 01 §14, A16 (`a16-department-subject-picker`) |
| 38 | **Storage orphan cleanup = daily cron(`0 0 1 * * * Asia/Seoul`) + `app.storage.orphan-cleanup.*` properties + `StorageClient.listOlderThan(grace)` 확장 + `FileVersionRepository.streamActiveStorageKeys()` (no `deleted_at` filter — trash 보호). per-row 실패 isolation + max-per-run cap=10000. summary audit `STORAGE_ORPHAN_CLEANED` 1건/run. default `enabled=false`(운영자 활성화). 운영 트리거 endpoint 미도입(backlog).** | **트리거 = daily cron, 운영자 enable** — A7 hard purge / share-expired-cron / permissions-expired-cron 일관 (`enabled=false` default). cron `0 0 1 * * *` (Asia/Seoul) — A7 purge가 자정에 돌므로 1시간 격차로 trash purge → orphan 발생 → orphan 잡 처리 순. **DB live set = `file_versions.storage_key` 전체 (NO `JOIN files WHERE deleted_at IS NULL`)** — 30일 trash 내 file의 version storage_key는 보존 대상이라 단순 stream으로 충분. A7 hard purge가 file row + file_versions cascade 삭제하면 다음 cron에서 자연 orphan으로 분류. ADR #31 deferred orphan(A7 hard purge가 audit `orphanStorageKeys`에 적재만 한 잔존 객체)과 동일 경로로 처리. **walk 대상** = LocalFs `{root}/{YYYY}/{MM}/*` 트리, `{UUID}` leaf만 candidate. 비-UUID name / 디렉토리 leaf / symlink는 skip + WARN log. **grace = mtime > NOW-24h skip** — in-flight 업로드(트랜잭션 timeout < 5분 가정) race 회피. **삭제 cap = `max-per-run:10000`** — A7 `MAX_PURGE_PER_RUN` 패턴. **per-row 실패 isolation** — 객체 1개 delete 실패는 ERROR log + 다음 candidate 진행. counters에 `failed` 증가. **Audit = summary 1건/run** = `STORAGE_ORPHAN_CLEANED` (target_type=`system`, target_id=`NULL`, metadata=`{scanned, candidates, deleted, failed, truncated, runId, durationMs}`). A7 `SYSTEM_PURGE_EXECUTED` 일관 — per-row spam 회피. **Lock = `@SchedulerLock` 미도입** — MVP single-instance 가정 (A7 패턴 일관). **Properties 네임스페이스 = `app.storage.orphan-cleanup.*`** — `app.*`(job-related) 일관, `ibizdrive.storage.*`(client config)와 분리. **신규 enum 추가만으로 호환** — V3 `audit_log.event_type` CHECK 미존재 (VARCHAR(50) 자유). 마이그레이션 0. **listOlderThan 시그니처 = `Stream<StorageObject> listOlderThan(Duration grace)`** — Stream lazy 보장으로 큰 트리에서 메모리 폭증 회피. caller(`try-with-resources`)로 close 책임. S3 impl(v1.x)은 ListObjectsV2 paginator로 자연 매핑. **권한 트리거 = 시스템 잡 only** — HTTP endpoint 미도입(운영 트리거는 backlog). ROI 검증 후 admin endpoint 별도 ADR. | 02 §5/§6, 03 §4.1, 04 §13, `storage-orphan-cleanup` |
| 39 | **M-RP.2 버전 복원 의미론 = 옵션 A (current_version_id 재지정, 새 version row 생성 안 함). 멱등(같은 versionId 재호출 시 audit 0). denormalized 메타(`files.size_bytes`/`files.mime_type`) 동기화 필수.** | **결정 = 옵션 A**: `POST /api/files/{fileId}/versions/{versionId}/restore`는 `files.current_version_id`만 target version으로 재지정. 새 version row 생성하지 않음(옵션 B 거부). **옵션 B 거부 사유**: 매 복원마다 storage 객체 복제 + version row 생성 → 큰 파일 반복 복원 시 storage/DB 비용 ↑ + version_number 단조 증가로 history 가독성 ↓. 옵션 A는 history 보존(기존 version row 그대로) + storage 비용 0 + version_number 의미 유지(업로드 순서). **트랜잭션** = `@Transactional` + `SELECT FOR UPDATE` (file 행 lock). file active + lock → version load + `version.fileId == fileId` 검증 → `file.currentVersionId == versionId`이면 멱등(audit emit 0, 현 file 반환) → 그 외 update + audit `VERSION_RESTORED` (`before={versionId: oldCurrent}`, `after={versionId: newCurrent}`). **권한** = `hasPermission(#fileId, 'file', 'EDIT')`. **다른 파일의 versionId** → 404 `FileNotFoundException`. **soft-deleted file** → 404. **자체 리뷰 보강 (sub-requirement)**: denormalized 메타 동기화 — `files.size_bytes`/`files.mime_type`은 `FileUploadService:214-217` invariant로 current version 메타와 일치해야 list view에서 stale 표시 회피. 옵션 A는 `current_version_id`만 바뀌므로 size/mime는 별도 update 필요. 자체 리뷰에서 누락 발견 후 `FileVersionMutationService`가 version load 시 size/mime도 함께 file row에 동기화하도록 보강. **frontend**: `api.downloadVersion(fileId, versionId)` = `<a href download>` 직접 호출(브라우저 stream 처리), `api.restoreVersion` = mutation, onSuccess에서 `qk.fileVersions(id)` + `qk.fileDetail(id)` invalidate. VersionsTab 각 row에 다운로드/복원 버튼, current 버전은 복원 비활성. **Audit emission 활성화** — `VERSION_DOWNLOADED`/`VERSION_RESTORED` 두 enum 정의됐으나 사용처 0이었음. 본 ADR로 emit 활성화(audit emit coverage 26 → 28). | 02 §7.6, 03 §4.1, 01 §17.5, M-RP.2 (`m-rp-rightpanel-completion`) |
| 40 | **M-RP.4 RP-2 정책 (file activity timeline 권한 범위) = `targetType="file"` + `targetId` 지정 + 호출자 파일 `READ` 보유 시 actor 제한 우회. 그 외 기존 정책(자기 actor만) 유지. AuditQueryFilters에 `targetType`/`targetId` nullable 필드 additive 추가 (M12 회귀 0).** | **요구**: RightPanel activity 탭은 파일 단위 audit timeline을 표시 — 파일 권한 보유자가 다른 사용자의 활동(업로드/다운로드/이동/공유 등)을 볼 수 있어야 UX 의미가 있음. 그러나 기존 audit query 정책은 actor 제한(자기 활동만, ADMIN/AUDITOR만 우회 가능). 이 두 요구를 정합. **결정 = file scope opt-in 우회**: query string에 `targetType=file` + `targetId=<UUID>` 둘 다 지정되고, 호출자가 해당 파일 `READ` 보유 시에만 actor 제한 우회. file scope 명시(어떤 파일에 대한 활동인지) + 권한 가드(READ 미보유 시 그냥 빈 결과 또는 본인 활동만)로 audit 노출 최소화. **재사용**: 권한 평가는 `IbizDrivePermissionEvaluator`/`PermissionService` 기존 호출 — 새 권한 enum 추가 0. **AuditQueryFilters 확장**: record 필드 2개 nullable 추가(`targetType: String?`, `targetId: UUID?`). `empty()` 갱신. WHERE 절 `(targetType IS NULL OR audit_log.target_type = :targetType) AND (targetId IS NULL OR audit_log.target_id = :targetId)` — 둘 다 null이면 기존 쿼리와 동일(M12 audit logs 페이지 회귀 0). **다른 targetType (folder/share/permission)** = 본 ADR 범위 외. 필요 시 별도 정책 ADR(folder는 EDIT? share는 owner?). **Audit emission 변동 0** — 본 ADR은 query/read 정책. **Frontend** = `api.listFileActivity(fileId)` → `?targetType=file&targetId=<id>&page=1&pageSize=20`. RightPanel activity 탭이 비활성일 때 fetch 차단(`enabled: tab === 'activity'`). | 02 §7.12, 03 §4, 01 §17.5, M-RP.4 (`m-rp-rightpanel-completion`) |
| 42 | **a1.5 email infra = `EmailService` 추상화 + `ConsoleEmailService(@Profile("!prod"))` + `SmtpEmailService(@Profile("prod"))` + Spring Mail starter. dev/test는 stdout 로깅, prod는 SMTP env vars(`SPRING_MAIL_HOST`/`PORT`/`USERNAME`/`PASSWORD`).** | **결정 = profile 분기 단일 인터페이스** — local/CI에서 SMTP 의존성 없이 메일 본문 검증 가능, prod 부팅 시 환경변수 누락이면 빈 wiring 실패로 즉시 가시화. 별도 이메일 발송 큐(SQS/Kafka)는 v1.x — MVP 비밀번호 분실 트래픽이 동기 발송으로 충분. 발송 실패는 `EmailDeliveryException`으로 표면화하되 caller 정책에 따라 swallow(anti-enumeration). HTML 템플릿/i18n 미도입 — text/plain 단일 본문 (KISS). **Anti-enumeration 한계**: timing leak(가입자만 SMTP 호출)은 본 트랙 범위 외. 동기 발송이 라운드트립 차이를 유발할 수 있어 v1.x rate-limit + 비동기 큐 트랙에서 재검토. | 03 §2.7, a1.5-email-infra |
| 43 | **a1.5 비밀번호 재설정 토큰 = SHA-256 hash 저장 + 30분 TTL + 1회 사용. 평문 토큰은 메일 링크에만, DB는 hash만. anti-enumeration: forgot 가입/미가입 동일 200, reset INVALID_TOKEN 사유 비공개.** | **결정 = SHA-256 hash + TTL + 1회 사용** — DB 유출 시 토큰 재사용 차단(평문 미저장). UUID 2개 join(64자) 평문 → SHA-256 hex(64자) 저장. **TTL = 30분** — 사용자 메일 확인 + 클릭 + 새 PW 입력 여유 + 시간 기반 brute-force 표면적 ↓. **1회 사용** = `password_reset_tokens.used_at` 컬럼 set 후 동일 토큰 재호출 INVALID_TOKEN. **다중 토큰 허용** — 같은 사용자가 forgot 여러 번 호출 시 모든 active 토큰 유효(마지막만 유효 정책 거부 — race/UX 부담). **rate-limit 미도입** — 본 트랙 범위 외, v1.x rate-limit 트랙. **POST /reset 성공 시 모든 세션 invalidate** (`FindByIndexNameSessionRepository.findByPrincipalName` → `deleteById` loop) — PW 변경이 기존 세션 무효화하는 보안 정책. **POST /change(인증)은 현재 세션 보존** — 사용자 강제 로그아웃 회피, 다른 기기 세션만 invalidate. **INVALID_TOKEN 사유 비공개** — 만료/사용됨/미존재 모두 동일 코드 (토큰 enumeration 방지). V8 마이그레이션이 `password_reset_tokens(id BIGSERIAL, user_id UUID FK CASCADE, token_hash VARCHAR(64) UNIQUE, expires_at, used_at, created_at)` + `idx_password_reset_tokens_active(expires_at) WHERE used_at IS NULL`. AuditEventType 신규 enum 2종 추가(`USER_PASSWORD_FORGOT_REQUESTED`, `USER_PASSWORD_RESET`) — `USER_PASSWORD_CHANGED`는 이미 존재. | 02 §2.x/§7, 03 §2.7/§4.1, a1.5-email-infra |
| 44 | **forgot rate limit = email(lower) + IP 두 키 독립 버킷, 60s 고정 윈도우, 한도 1회/키, OR 차단. in-memory `ConcurrentHashMap` (LoginAttemptTracker 패턴 mirror — ADR #23). 외부 의존 0. 429 + `Retry-After` 헤더 + `RATE_LIMIT_EXCEEDED` envelope (docs/02 §8 기존 코드 재사용).** | **결정 = single-instance in-memory** — `LoginAttemptTracker`(ADR #23)와 동일 가정/패턴 답습으로 코드 표면적·운영 부하 최소. Bucket4j(외부 lib 추가)·Redis(인프라 도입)·슬라이딩 윈도우(YAGNI) 모두 거부. **두 키 OR 차단** — 같은 email(다른 IP) 또는 같은 IP(다른 email) 한 쪽만 한도 초과해도 차단. AND 조합은 NAT 환경에서 회피 쉬움(다른 사용자 가장으로 IP 측 우회), OR이 보안 우선. **차단 시 윈도우 미연장** — 차단된 호출은 last-hit Instant를 갱신하지 않아 봇 폭주가 윈도우를 무한 연장하지 못함. **lazy 만료** — 60s 경과 시 자연 통과 + 갱신, 별도 cleanup 스레드 미운영(ADR #23 일관, KISS). **IP 추출** — `X-Forwarded-For` 첫 값 → fallback `getRemoteAddr()`. 사내 베타에서 reverse proxy 경유 가정. spoof 가능 — trusted proxy 정책은 별도 트랙(한계 명시). **anti-enumeration 정합** — 429는 가입 여부 무관 동일 limiter 경로(통과/차단은 키 hit 기준이지 user 존재 기준 아님). **Audit 미발행** — rate-limited는 리소스 도메인 아니라 메타. `WARN` 로그(email 마스킹 `a***@b.com` + IP) 1줄로 충분, audit_log 오염 회피. **Login surface 비범위** — `/api/auth/login`은 `LoginAttemptTracker` 5/15min lockout으로 별도 보호 중. /reset/change/signup도 별도 트랙. **한계** — (a) 다중 인스턴스 시 카운터 N배 — 인터페이스 추출은 v1.x Redis 도입 시점. (b) NAT 공유 IP 환경에서 다른 사용자 영향 — 사내 베타 가정 + 60s/1회는 정상 사용 마찰 없음. (c) `X-Forwarded-For` spoof 가능. **a1.5 closure (`PasswordResetService.java:45`) 잔여 항목 close.** | 02 §7.4/§8, 03 §2.7, BETA-RELEASE.md §5, auth-forgot-rate-limit |
| 41 | **auth-pages = 셀프 가입(`POST /api/auth/signup`) + first-user-ADMIN + `/login`·`/signup` 페이지 + (explorer) 401 가드. ADR #21(관리자 초대 only) supersede.** | **결정 = self-signup MVP 활성화** (BETA 차단 해제) — 운영자가 `POST /api/admin/users`로 모든 user를 사전 생성해야 첫 사용자가 시스템에 들어올 수 있던 ADR #21 제약을 해소. 사내 시스템이지만 BETA 진입 자체가 첫 가입자 부재로 막혀있어 KISS+§3 원칙 9(문제 은폐 금지)에 따라 self-signup endpoint 도입. **first-user-ADMIN 부트스트랩** = `userRepository.count() == 0`이면 새 사용자 ROLE=ADMIN, 그 외 MEMBER. 빈 DB 첫 호출만 ADMIN 부여 → 초기 admin 시드 의존성 제거. race(동시 두 요청)는 MVP 가정 single-instance + tx 직렬화로 사실상 차단(엄밀 보장은 advisory lock — v1.x). **응답 = login과 동일 shape** (`{user, departments, roles, effectivePermissionsCacheKey}`) + auto-session(`AuthService.establishSession` extract — login/signup 공통 helper, `changeSessionId()` 호출). 가입 성공 = 자동 로그인 = 즉시 `/files`. **CSRF 정책** = `/api/auth/signup`은 `csrf().ignoringRequestMatchers` + `permitAll()`. 첫 호출이라 token preflight 비용/UX 마찰 회피. login/logout은 ADR #12 그대로(double-submit). **Validation = Bean Validation** (`@NotBlank @Email @Size(max=254)` email, `@NotBlank @ValidPassword` password [auth-password-policy 2026-05-04 closure: ADR #19 본문 회복 — 12자 이상 + 영문+숫자 + 공백 금지], `@NotBlank @Size(min=1, max=100)` displayName). email은 trim+lowercase 후 저장(중복 방지 일관성 — `users.email CITEXT` UNIQUE 의존). **DUPLICATE_EMAIL** = 409 `{code:"CONFLICT", reason:"DUPLICATE_EMAIL"}` (auth flat envelope). VALIDATION 400은 standard envelope. **Audit emission** = `USER_REGISTERED("user.registered")` enum 추가 + `UserRegisteredEvent` record + `AuthAuditListener.onRegistered`. 이벤트 발행은 `SignupService` 내부, listener가 `@EventListener` REQUIRES_NEW로 audit_log 기록. login `AuthenticationSuccessEvent` 패턴 일관. **Frontend** = `/login`·`/signup` 페이지(라우트 그룹 `(auth)`, chrome 없는 미니멀 layout) + `(explorer)` AuthGuard(useMe 401→`/login?next=...` replace). useMe는 401을 null로 흡수(retry false, staleTime 60s). useLogin/useSignup은 onSuccess에 `qk.authMe()` setQueryData + `qk.all` invalidate. useLogout은 onSettled clear(의도 기반 — 401에도 캐시 비움). 사이드바 하단 UserMenu(displayName + 로그아웃 버튼). 로그인 페이지는 `useSearchParams` Suspense wrap(Next.js 15 prerender 호환). | 03 §2, §4, 02 §7.4, A1.4 (`auth-pages`) |
