...
| 34 | **A10 shares endpoint = `shares` 별도 row + `permissions` row 위 메타. POST `/api/files/:id/share`(file 단건 — folder share endpoint 미도입, schema 양립 backlog). DELETE `/api/shares/:shareId` = `revoked_at` set + permission row delete + audit `share.revoked` 단일 발행(`permission.revoked`는 emit 안 함, 이중 발행 회피). request wire format = preset 5종 + subject 4종 lower-case (Preset.java/PermissionService 진실의 출처). `expires_at` 컬럼 V6 추가. with-me MVP = `subject_type='user'` 매칭만(department/role/everyone backlog). `SHARE_CREATED`/`SHARE_REVOKED` audit 첫 활성화. `SHARE_EXPIRED` cron + folder share endpoint + SSE emission 모두 별도 트랙 deferred(audit-only).** | **shares 테이블 = SRP 분리** — ADR #28 본문 "preset 4값(share 제외, share 는 별도 shares 테이블)"의 의미 정합: `permissions.preset`은 5종(`read`/`upload`/`edit`/`share`/`admin`) 그대로(Preset.java 진실의 출처) — `share` preset = "공유받은 사람의 권한 셋"(read+upload+edit+move+download+delete+share), `shares` 테이블 = 공유 메타(message/expiresAt/revoke 추적) + `permission_id` FK로 권한 row와 1:1 연결. 권한 row에 message/revoked_at 컬럼 추가 대안은 거부(권한과 공유 메타 책임 혼합). **request wire format 정정** — docs/02 §7.9 본문 `'VIEW'\|'EDIT'\|'FULL'`/`'USER'\|'DEPT'`는 코드와 case·집합 모두 drift → A10.0에서 lower-case + Preset.java 5종 + PermissionService 4 subject로 정합. **revoke 시 permission row delete + share `revoked_at` set** — 공유받은 사람이 file 접근 즉시 회수(권한 row 잔존 시 보안 누설). audit 이중 발행 회피 — `share.revoked` metadata에 `permissionId` 보존하여 추적 가능. **`expires_at` 컬럼** — frontend permission expiresAt UX 패리티. `permissions.expires_at`(V5)과 의미 분리: share만 expire되어도 permission row는 살아있을 수 있음 — `SHARE_EXPIRED` cron(별도 트랙)이 도과 row를 `revoked_at`으로 set + permission row delete. **with-me MVP scope** — `subject_type='user'` 매칭만 결과 포함. department(LTREE 후손 join)/role/everyone subject는 frontend 미사용 + 쿼리 비용 ↑ → 별도 ADR. **folder share endpoint 미도입** — `POST /api/folders/:id/share` 신설은 V_(컬럼 추가 없음) + ShareCommandService overload만으로 hook. **편법 아닌 명시적 deferred** (CLAUDE.md §3 원칙 9). | 02 §2.7/§7.9, 03 §3.1, A10 (`a10-shares`) |
