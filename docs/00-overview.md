...
| 21 | **사용자 등록 = 관리자 초대 only (셀프 가입 금지)** | 사내 시스템 → 셀프 가입 부적절. MVP는 (a) Flyway 시드로 admin 1명 + (b) `POST /api/admin/users` (admin이 email/임시PW 지정)로 user 생성. 첫 로그인 시 PW 변경 강제. 초대 메일은 A1.5(이메일 인프라 도입 시). **Status: Superseded by ADR #41 (2026-05-02)** — auth-pages 트랙에서 셀프 가입 + first-user-ADMIN 부트스트랩으로 정정. 초기 admin 시드 의존성 제거(빈 DB 첫 가입자가 ADMIN). 운영자 초대(`POST /api/admin/users`)는 v1.x reserve. | 03 §2.8, 02 §7.12 |
