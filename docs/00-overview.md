...
| 29 | **`file_versions` entity/repository/CRUD endpoint A5 이월 — V5 schema에는 테이블만 도입** | YAGNI (CLAUDE.md ULTIMATE INVARIANTS 2). 버전 관리 UI/API(이력 탭, 복원, 비교)는 별도 기능 — A4 MVP CRUD(create/rename/move/delete/restore)와 결합 없음. V5 schema는 `file_versions` 테이블 + DEFERRABLE FK `files.current_version_id` 도입(backup V5와 동일) → A5에서는 entity/repository/endpoint만 추가하면 됨. **보장사항**: (a) `files.id`는 mutation/restore 시 stable, (b) `storage_key` 교체 시 `file_versions` 행 추가 가능 구조 유지, (c) `current_version_id` NULL 허용 그대로 유지(MVP는 최신 버전 단일 가정, A5에서 NOT NULL 제약 추가 검토). codex backup `FileVersion.java`는 A5 부트스트랩 시점에 reference로 재검토 — A4에서 cherry-pick 금지. **A5 진입 (2026-04-29)**: FileVersion entity + FileVersionRepository + GET /api/files/:id/versions endpoint 도입 시작. close는 A5.3 commit hash로 갱신 예정 (단일 PR `feature/a5-file-versions`). | 02 §2.5, A4.1, A5 진입 |
