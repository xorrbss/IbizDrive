...
| 21 | **사용자 등록 = 관리자 초대 only (셀프 가입 금지)** | 사내 시스템 → 셀프 가입 부적절. MVP는 (a) Flyway 시드로 admin 1명 + (b) `POST /api/admin/users` (admin이 email/임시PW 지정)로 user 생성. 첫 로그인 시 PW 변경 강제. 초대 메일은 A1.5(이메일 인프라 도입 시). **Status: Superseded by ADR #41 (2026-05-02)** — auth-pages 트랙에서 셀프 가입 + first-user-ADMIN 부트스트랩으로 정정. 초기 admin 시드 의존성 제거(빈 DB 첫 가입자가 ADMIN). 운영자 초대(`POST /api/admin/users`) admin 트랙도 closure 완료(2026-05-03, `admin-invite-email`) — admin이 email/displayName/role 지정, 임시 PW(16자 SecureRandom alnum+일부 특수)는 백엔드 생성·BCrypt 해시 저장·invite 메일 본문에만 노출(응답 DTO/로그 비포함), `mustChangePassword=true`로 첫 로그인 후 강제 변경 chain. `@PreAuthorize("hasRole('ADMIN')")` + Bean Validation + audit `ADMIN_USER_CREATED` emit. | 03 §2.7/§2.8/§2.10, 02 §7.4 |
