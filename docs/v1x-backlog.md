...
| **2인 승인 framework Phase 3+** (handler/cron/admin UI) | M | Phase 1+2+2b 완료 (#219 + #223 + 본 PR) | BETA §7 / ADR #47 / progress.md 2026-05-13 | Phase 1 data (#219) + Phase 2 state machine (#223) + Phase 2b controller 5 endpoint (본 PR). 잔여: Phase 3 retention/role/trash-purge `AdminApprovalActionHandler` 구체 구현 + 기존 controller per-action 게이트 hook (`app.dual-approval.retention-change.enabled` 등) / Phase 4 admin UI (`/admin/approvals` 페이지 + frontend hook) / Phase 5 expiration cron (`AdminApprovalExpirationJob`) + email listener (secondary 알림) |
