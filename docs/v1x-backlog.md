...
| ~~파일 타입 아이콘 fidelity (icons.jsx §ICONS L4~73)~~ | — | — | ✓ 2026-05-12 design-sweep-file-type-icons (PR #TBD) | **closure** — `FileTypeIcon` 신규(10 kind 컬러 SVG 1:1 inline, doc/pdf/sheet/slides/image/video/figma/code/archive + folder) + `fileIconFor`(lucide 5종 단색) → `fileIconKind` mime→kind 매핑 rewrite. FileRow/FileCard 호출자 swap. backend 변경 0, 132 tests PASS |
