...
| **2인 승인 framework Phase 3b+** (role/purge handler + admin UI + cron) | M | Phase 1+2+2b+3a 완료 (#219 + #223 + #226 + 본 PR) | BETA §7 / ADR #47 / progress.md 2026-05-13 | Phase 1 data (#219) + Phase 2 state machine (#223) + Phase 2b controller (#226) + Phase 3a retention_change handler + AdminTrashPolicyController 게이트 + 202 APPROVAL_REQUIRED 매핑 (본 PR). 잔여: Phase 3b `RoleChangeApprovalHandler` + `AdminUsersController` 게이트 / Phase 3c `TrashPurgeApprovalHandler` + `TrashController.purge` 게이트 / Phase 4 admin UI (`/admin/approvals` 페이지 + frontend hook) / Phase 5 expiration cron (`AdminApprovalExpirationJob`) + email listener (secondary 알림) |
