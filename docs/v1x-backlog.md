...
>
> **Last Updated**: 2026-05-15 (quick-action-dialog closure — WelcomeHeader 업로드/새 폴더)
...
| ~~FileRow starred 배지 wiring (P2a) — frontend~~ | — | — | ✓ 2026-05-14 file-favorites-p2a-frontend (PR #241) + 2026-05-15 ultrareview follow-up (PR #260) + 2026-05-15 grid-star fidelity (PR #263) | **closure (Option D zip-faithful)** — `useToggleStar` hook(optimistic + rollback) + `api.toggleStar` 단일 helper(file/folder × POST/DELETE 4 endpoint) + `invalidations.afterStarToggle` + `BreadcrumbWithStar` wrapper(`isStarred`/`onToggleStar` props 주입) + `FileRowActionMenu` 즐겨찾기 항목. **sleeping bug 동시 해소** — `getFilesInFolder` mapper가 4 배지 필드(starred/restricted/shareCount/itemsCount) 전부 drop하던 것을 동시 propagate → design-sweep-phase-2 4 배지 실 화면 활성화. backend: `FolderDetailResponse.starred` 필드(Breadcrumb 현재 폴더 starred 조회용). 1479 frontend test + backend gradle BUILD SUCCESSFUL + CI 풀그린. **PR #260 follow-up** — ultrareview nit 2건 정정: `useToggleStar` folder 분기 `cancelQueries(qk.folder(id))` race window 제거(`useCurrentFolder` staleTime 60s + refetchOnWindowFocus 와 충돌) + `BreadcrumbWithStar` `onError` toast wiring(`FileRowActionMenu` 동등 패턴). 2 파일 +13/-6. **PR #263 grid-star fidelity** — design zip §grid-star (components.jsx L466 + styles.css L744~753) 가 `FileRow` 만 반영되고 `FileCard` 미반영된 design drift 정정: outer card `relative` + `item.starred` 시 `absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-surface-1 text-warn shadow-sm` 별 배지 + 회귀 가드 1건. list↔grid 전환 시 starred 표시 일관성 |
| ~~2인 승인 framework Phase 4 FE follow-ups~~ | — | — | ✓ 2026-05-14 dual-approval Phase 4 FE follow-up 풀세트 (#219+#223+#226+#228+#229+#230+#233+#235+#236+#238 + 본 PR 202 handling) | **closure (Phase 4 FE 풀세트)** — backend Phase 3 (Tier 0 3 handler + cron) + Phase 4 admin UI (#235 `/admin/approvals` 페이지 + queryKeys/api/hooks/AdminTabBar) + email listener (#236 `AdminApprovalEmailListener` 4 transition 한국어 템플릿) + AdminTabBar pending count 배지(#238 `useAdminPendingApprovalsCount`) + **202 APPROVAL_REQUIRED 처리(본 PR)**. 본 PR: `ApprovalRequiredError` + `parseApprovalRequired` + `throwIfApprovalRequired` API wrapper + 3 mutation hook onError 분기 + `showApprovalRequiredToast` (sonner info + action 버튼 → `/admin/approvals/:id`) + `RetentionPolicyEditor` dialog 닫힘/draft 유지 + docs/01 §14.6 패턴 documentation. 잔여 없음 — gate=OFF 기본 동작은 200 OK + entity (기존 흐름 유지) |
...
| ~~User Home Dashboard (root `/`)~~ | — | — | ✓ 2026-05-14 user-home-dashboard 풀세트 (PR #246 + #248 + #252 + #253) | **closure (4 위젯 모두 actionable)** — (a) PR #246 root `/` redirect → personal dashboard 4 위젯(Welcome/Starred/Quota/SharedWithMe) + 신규 endpoint `GET /api/me/shared-with-me`. ADR #48 신규. favorites-list 트랙(PR #243) 산출물 reuse. (b) **follow-up 풀세트**: PR #248 SharedWithMeCard row click → explorer navigation + backend response `workspace`/`navigationFolderId` 확장. PR #252 StarredCard row click(backend 변경 0). PR #253 WelcomeHeader "내 워크스페이스 →" 진입 link. (c) recent files 는 ADR #9 (`FILE_VIEWED`) blocker 로 v1.1 deferral. 업로드/새 폴더 quick action dialog 자동 오픈은 별도 트랙으로 분리(quick-action-dialog 행 참조) |
| ~~WelcomeHeader 업로드/새 폴더 quick action~~ | — | — | ✓ 2026-05-15 quick-action-dialog (PR #<TBD>) | **closure** — PR #253 의 단일 navigation link 를 spec §3.1 의 정식 두 버튼 패턴으로 전환. 업로드 = in-place file picker(user gesture 보존) + `useUpload.enqueue` + `router.push(workspaceRoot)`. 새 폴더 = `router.push(workspaceRoot?action=new-folder)` → `useQuickActionParam` hook 이 ClientFilesPage 에서 query 감지 + `CreateFolderDialog` mount + 1-shot consume `router.replace`(`?action` 만 제거, 다른 query 보존). URL convention `?action=new-folder` 한 케이스만 도입(KISS). spec: `docs/superpowers/specs/2026-05-15-quick-action-dialog-design.md` |
