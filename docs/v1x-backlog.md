...
| **2인 승인 framework Phase 4** (admin UI + email) | M | Phase 1~3 완료 (#219 + #223 + #226 + #228 + #229 + #230 + 본 PR) | BETA §7 / ADR #47 / progress.md 2026-05-13 | Phase 3a retention (#228) + 3b role_change (#229) + 3c trash_purge (#230) + 3d expiration cron `PendingAdminApprovalExpirationJob` + V21 `cron_policy.admin.approval.expire` + /admin/system viewer 5번째 entry (본 PR — Tier 0 + cron closure). 잔여: Phase 4 admin UI (`/admin/approvals` 페이지 + pending list + approve/reject/cancel UX + frontend hook) / email listener (secondary 알림, EmailService 재사용 ADR #45) |
