...
>
> **Last Updated**: 2026-05-15 (file-favorites ultrareview follow-up closure, PR #260)
...
| ~~FileRow starred 배지 wiring (P2a) — frontend~~ | — | — | ✓ 2026-05-14 file-favorites-p2a-frontend (PR #241) + 2026-05-15 ultrareview follow-up (PR #260) | **closure (Option D zip-faithful)** — `useToggleStar` hook(optimistic + rollback) + `api.toggleStar` 단일 helper(file/folder × POST/DELETE 4 endpoint) + `invalidations.afterStarToggle` + `BreadcrumbWithStar` wrapper(`isStarred`/`onToggleStar` props 주입) + `FileRowActionMenu` 즐겨찾기 항목. **sleeping bug 동시 해소** — `getFilesInFolder` mapper가 4 배지 필드(starred/restricted/shareCount/itemsCount) 전부 drop하던 것을 동시 propagate → design-sweep-phase-2 4 배지 실 화면 활성화. backend: `FolderDetailResponse.starred` 필드(Breadcrumb 현재 폴더 starred 조회용). 1479 frontend test + backend gradle BUILD SUCCESSFUL + CI 풀그린. **PR #260 follow-up** — ultrareview nit 2건 정정: `useToggleStar` folder 분기 `cancelQueries(qk.folder(id))` race window 제거(`useCurrentFolder` staleTime 60s + refetchOnWindowFocus 와 충돌) + `BreadcrumbWithStar` `onError` toast wiring(`FileRowActionMenu` 동등 패턴). 2 파일 +13/-6 |
