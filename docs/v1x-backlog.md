...
>
> **Last Updated**: 2026-05-13 (RightPanel P_panel-A/B closure + P_panel-C 등록)
...
| RightPanel 디자인 fidelity sub-PR A+B (backend extension + detail rows) | — | — | ✓ 2026-05-13 (PR #218 P_panel-A + PR #220 P_panel-B) | **closure** — `FileDetailResponse` envelope(file+owner+sharedWith+folderPath) + `UserBriefDto`/`SubjectGrantBriefDto` + `FileQueryService.loadDetail` 확장(PermissionService.listPermissions 재사용 + folderPath walk). FE detail tab에 소유자/공유/경로 3 row 추가 (`AvatarBadge`/`SharedWithStack`/`FolderPathInline` inline). 20/20 RightPanel test PASS |
| RightPanel sub-PR C (헤더 액션 + PreviewCard + folderPath 링크화) | S~M | sub-PR A+B 완료 | 2026-05-13 P_panel-B 종료 후 잔여 | 헤더 액션 3개(다운로드/공유/더보기 메뉴 — 기존 useUpload/share/etc UI store wiring) + `<PreviewCard>` (mime별 미리보기 — preview API source 확인 필요) + folderPath 링크화 (workspace 라우팅 컨텍스트 빌더). viewCount row는 ADR #9(`FILE_VIEWED` audit emit) blocker로 별도. 진입 전 PR #221(co-session "풀세트") 결과 확인 — 일부 또는 전체 겹칠 수 있음 |
...
| **2인 승인 framework Phase 2+** (service/controller/audit/hook/admin UI/cron) | M~L | Phase 1 완료 (본 트랙) | BETA §7 / ADR #47 / progress.md 2026-05-13 | data layer 완성 (V20 + `PendingAdminApproval` entity + repository + integration test 12건). 잔여: Phase 2 service + controller (`POST/approve` `/reject` `DELETE/cancel`) / Phase 3 audit emit 4종 + 에러 코드 4종 (`APPROVAL_REQUIRED`/`APPROVAL_SELF`/`APPROVAL_NOT_FOUND`/`APPROVAL_ALREADY_DECIDED`) + Permission.APPROVE_ADMIN_ACTION / Phase 4 retention/role/trash-purge controller hook + per-action config 게이트 / Phase 5 admin UI (`/admin/approvals` 페이지) / Phase 6 expiration cron + email listener |
