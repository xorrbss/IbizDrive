...
| **2인 승인 framework Phase 2b+** (controller/handler/cron/admin UI) | M | Phase 1+2 완료 (본 트랙 #219 + 본 PR) | BETA §7 / ADR #47 / progress.md 2026-05-13 | Phase 1 data layer (#219) + Phase 2 state machine 완성 — service 5 transition + audit emit 4종 + 4 exception handlers + Permission.APPROVE_ADMIN_ACTION + AuditTargetType.ADMIN_APPROVAL + Service Mockito test 17건. 잔여: Phase 2b `AdminApprovalController` 5 endpoint + DTOs / Phase 3 retention/role/trash-purge `AdminApprovalActionHandler` 구현 + 기존 controller per-action 게이트 hook / Phase 4 admin UI (`/admin/approvals` 페이지) / Phase 5 expiration cron + email listener |
