...
| **2인 승인 framework Phase 3c+** (purge handler + admin UI + cron) | S~M | Phase 1+2+2b+3a+3b 완료 (#219 + #223 + #226 + #228 + 본 PR) | BETA §7 / ADR #47 / progress.md 2026-05-13 | Phase 3a retention (#228) + Phase 3b role_change handler + `AdminUserController.patch` 게이트 + `AdminUserService.get(UUID)` 추가 (본 PR). 잔여: Phase 3c `TrashPurgeApprovalHandler` + `TrashController.purge`/`AdminTrashController.bulk` 게이트 / Phase 4 admin UI (`/admin/approvals` 페이지 + frontend hook) / Phase 5 expiration cron (`AdminApprovalExpirationJob`) + email listener (secondary 알림) |
