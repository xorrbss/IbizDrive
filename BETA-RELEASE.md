...
| audit emit coverage | 48 enum 중 41 emit (~85%) — `USER_REGISTERED` (auth-pages) + `USER_PASSWORD_FORGOT_REQUESTED` / `USER_PASSWORD_RESET` / `USER_PASSWORD_CHANGED` (a1.5-email-infra) + `ADMIN_USER_CREATED` (m-admin-entry-rewrite) + `ADMIN_USER_DEACTIVATED` / `ADMIN_ROLE_CHANGED` (admin-user-mgmt) + `ADMIN_USER_UPDATED` (admin-user-search-update Wave 1 T1) + `AUDIT_EXPORTED` (audit-export-endpoint Wave 1 T2; audit-export-json 트랙으로 `metadata.format` 동적화 `csv\|json`, 2026-05-08) + `ADMIN_DEPARTMENT_CREATED` / `ADMIN_DEPARTMENT_UPDATED` / `ADMIN_DEPARTMENT_DEACTIVATED` (admin-department-crud Wave 2 T4) + `ADMIN_CRON_TOGGLED` (admin-cron-policy-toggle, 2026-05-08 — `PUT /api/admin/system/cron/{key}`) 신규 emit. **미emit 7개는 §7 deferred 매핑 (`audit-emit-gap-mapping` closure, 2026-05-05; T2 갱신 2026-05-06) — 누락(버그) 0건** |
