...
Last Updated: 2026-05-13 (+ quota mutation Phase 6 closure: hard delete decrement, monotonic 한계 해소)
Source: `mvp-qa-security-week-11-12` 트랙 closure + `feature/mvp-prod-profile` 트랙 (application-prod.yml + cron 4종 활성화) + `m-rp-rightpanel-completion` 트랙 closure + `auth-pages` 트랙 closure (셀프 가입 + first-user-ADMIN, ADR #41) + `auth-must-change-pw` 트랙 closure (ADR #21 §2.7) + `auth-forgot-rate-limit` 트랙 closure (ADR #44) + `m-admin-entry-rewrite` 트랙 closure (admin shell + `POST /api/admin/users`, ADR #21) + `auth-password-policy` 트랙 closure (ADR #19 본문 회복) + `email-async` 트랙 closure (`@Async EmailService`, ADR #45) + `admin-user-search-update` (Wave 1 T1 — `ADMIN_USER_UPDATED` emit, #59) + `audit-export-endpoint` (Wave 1 T2 — `GET /api/admin/audit/export` + `AUDIT_EXPORTED` emit) + `admin-department-crud` (Wave 2 T4 — `/admin/departments` + 3 emit) + `admin-permission-matrix` (Wave 2 T5 — `GET /api/admin/permissions` + `/admin/permissions` read-only viewer) + `wave2-t6-folder-items-wire` (Wave 2 T6 — `GET /api/folders/{id}/items` + `GET /api/files/{id}` + frontend mock 일괄 제거) + `admin-dashboard` 트랙 closure (`GET /api/admin/dashboard/summary` + KPI 그리드, 2026-05-07) + `wave2-t9-admin-global-trash` (Wave 2 T9 — `GET /api/admin/trash` + `/admin/trash/all` viewer, 2026-05-07) + `audit-export-json` (Wave 1 T2 follow-up — `GET /api/admin/audit/export?format=csv|json`, 2026-05-08) + `admin-cron-toggle` (Wave 2 closure 후속 — `PUT /api/admin/system/cron/{key}`, ADMIN-only, 2026-05-08) + Wave 2 trash follow-ups (subtree-size #103 / restore-conflict-dialog #105 / export-filename-timestamp #107 / format-size #111 / policy-viewer #114 / original-path #120) + Wave 2 T5 follow-up (admin-permission-revoke 단일 row 철회 #123) + CSRF 일관화 sweep (#115 / #121 / #126 / #165 / #184) + spec 정합 (legal-hold-design v2.x #113 / v1x-confirm-2admin-design v1.x #124 / permission-matrix #149 / quota-spec-drift #185) + dev-preview-stabilization T1~T6 (#150~#153 — login-401-csrf fix + Playwright signup/login e2e + application-local.yml + seed SQL + local-dev.md) + design-refresh-admin T7/T8 (#154/#156/#158/#159 — admin chrome 8탭 + V16 admin team endpoints + `TEAM_UPDATED` emit + /admin/teams 프론트) + team-centric-pivot (Plan A Phase 1+2 핸드오프 9/30 task + Plan A2 team-domain-hardening `TEAM_ARCHIVED`/`TEAM_RESTORED` emit + Plan C share subject_type='team' #140) + grant-permission-dialog Phase A/B #157 (subject=everyone, Phase C/D는 v1.x) + design-handoff G1~G8 closure (#148/#168/#171/#172/#174/#175/#177/#181 — TopBar 3-col grid + SearchBar ⌘K + cheatsheet 모달 + density 토글 3종 + FileTable 6열 + kbd platform 분기 + shortcuts help button) + file-row-action-menu #183 (G4 컨텍스트 메뉴 wiring — D5 결정 번복) + trash-retention-mutation Phase A/B/C #167/#169/#173 (V17 + TrashPolicyService + PUT endpoint + audit listener + frontend editor) + no-mobile-support-policy #179 (CLAUDE.md §3 원칙 13 — G7 폐기 공식화) + quota mutation 6-phase 전체 closure (#185 spec / #186 V18 컬럼 / #198 Phase 3 backend `AdminUserQuotaService` + `GET/PUT /api/admin/users/{id}/quota` + `ADMIN_QUOTA_CHANGED` emit / #203 Phase 4 frontend `/admin/members` quota 컬럼 + `AdminUserQuotaCell` + `useAdminUpdateUserQuota` + summary response 필드 추가 / #204 Phase 5 `UserQuotaEnforcer.consumeOrThrow` + `FileUploadService.upload` 진입 가드 + storage_used 증분 + 413 QUOTA_EXCEEDED + GlobalExceptionHandler 매핑 / Phase 6 본 트랙 `User.releaseStorage` + `UserQuotaEnforcer.release` + `TrashPurgeService.purgeFile`/`purgeFolder` + `HardPurgeService` 배치 cron 모두 wire — owner별 file_versions sizeBytes 합산 후 storage_used 감소, monotonic 한계 해소)
...
| audit emit coverage | **58 enum 중 52 emit (~90%)** — `USER_REGISTERED` (auth-pages) + `USER_PASSWORD_FORGOT_REQUESTED` / `USER_PASSWORD_RESET` / `USER_PASSWORD_CHANGED` (a1.5-email-infra) + `ADMIN_USER_CREATED` (m-admin-entry-rewrite) + `ADMIN_USER_DEACTIVATED` / `ADMIN_ROLE_CHANGED` (admin-user-mgmt) + `ADMIN_USER_UPDATED` (admin-user-search-update Wave 1 T1) + `AUDIT_EXPORTED` (audit-export-endpoint Wave 1 T2; audit-export-json 트랙으로 `metadata.format` 동적화 `csv\|json`, 2026-05-08) + `ADMIN_DEPARTMENT_CREATED` / `ADMIN_DEPARTMENT_UPDATED` / `ADMIN_DEPARTMENT_DEACTIVATED` (admin-department-crud Wave 2 T4) + `ADMIN_CRON_TOGGLED` (admin-cron-policy-toggle, 2026-05-08 — `PUT /api/admin/system/cron/{key}`) + **`TEAM_CREATED` / `TEAM_MEMBER_ADDED` / `TEAM_MEMBER_REMOVED` / `TEAM_MEMBER_ROLE_CHANGED` (team-centric-pivot Plan A Phase 1+2) + `TEAM_ARCHIVED` / `TEAM_RESTORED` (Plan A2 team-domain-hardening) + `TEAM_UPDATED` (admin-teams T8-P3 #156, 2026-05-10) + `ADMIN_RETENTION_CHANGED` (trash-retention-mutation Phase B #169, 2026-05-11) + `ADMIN_QUOTA_CHANGED` (quota-phase3 본 트랙 2026-05-12)** 신규 emit. **미emit 6개는 §7 deferred 매핑 (`audit-emit-gap-mapping` closure, 2026-05-05; T2 갱신 2026-05-06) — 누락(버그) 0건** |
