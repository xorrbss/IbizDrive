 ...
 * audit-export-json 트랙으로 format 분기).
 *
 * - 필터(행위자/이벤트/날짜) + 페이지네이션 + CSV/JSON export (server-side full-result)
 ...
 * - CSV/JSON export는 `GET /api/admin/audit/export?format=csv|json`. 현재 필터를 그대로 query string으로
 *   전달, 브라우저 anchor download로 트리거. 권한 가드는 backend `@PreAuthorize(AUDITOR or ADMIN)`이
