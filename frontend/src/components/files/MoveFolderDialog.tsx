...
export function MoveFolderDialog() {
  ...
  // URL → workspace scope 파생 (department / team만 — shared는 이동 컨텍스트 없음)
  const scope =
    ws && ws.section !== 'shared' && ws.workspaceId
      ? { type: ws.section as 'department' | 'team', id: ws.workspaceId }
      : null

  // 현재 workspace의 root folder 정보
  const root = (() => {
    if (!scope) return null
    if (scope.type === 'department') {
      const dept = workspaces?.department
      return dept && dept.id === scope.id
        ? { id: dept.rootFolderId, name: dept.name }
        : null
    }
    // team
    const team = workspaces?.teams.find((t) => t.id === scope.id)
    return team ? { id: team.rootFolderId, name: team.name } : null
  })()

  // sourceFolderId 캐시에서 ids 중 폴더인 것 추출 — invalidIds 구성용
  const cached = qc
  ...
  const handleConfirm = () => {
    ...
    // close() 즉시 호출 — ClientFilesPage에서 항상 mount되어 있으므로
    // hook-level onSuccess/onError 콜백은 mutation 완료 시 정상 실행됨.
    close()
  }
