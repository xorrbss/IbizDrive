...
class TrashQueryServiceTest {
    ...
    void list_emptyRepos_returnsEmptyPage() {
        ...
        when(fileRepository.findTrashedPage(any(), any(), anyInt())).thenReturn(List.of());
        when(folderRepository.findTrashedPage(any(), any(), anyInt())).thenReturn(List.of());

        TrashPage page = service.list(ACTOR, Role.ADMIN, null, null, null);
    ...
    void list_typeFile_skipsFolderRepo() {
        ...
        service.list(ACTOR, Role.ADMIN, null, TrashItemType.FILE, null);

        verify(fileRepository).findTrashedPage(any(), any(), anyInt());
    ...
    void list_typeFolder_skipsFileRepo() {
        ...
        service.list(ACTOR, Role.ADMIN, null, TrashItemType.FOLDER, null);

        verify(folderRepository).findTrashedPage(any(), any(), anyInt());
    ...
    void list_admin_returnsAllItemsSortedByDeletedAtDesc() {
        ...
        when(permissionService.effectivePermissions(Role.ADMIN))
            .thenReturn(EnumSet.allOf(Permission.class));
        when(fileRepository.findTrashedPage(any(), any(), anyInt()))
            .thenReturn(List.of(newFile(fileId, "doc.pdf", t0.minusSeconds(10))));
        when(folderRepository.findTrashedPage(any(), any(), anyInt()))
    ...
    void list_member_filtersByResourceLevelGrant() {
        ...
        TrashPage page = service.list(ACTOR, Role.MEMBER, null, null, null);
    ...
    void list_hasMore_emitsNextCursorMatchingLastItem() {
        ...
        TrashPage page = service.list(ACTOR, Role.ADMIN, null, TrashItemType.FILE, 1);
    ...
    void list_invalidCursor_throwsIllegalArgument() {
        org.assertj.core.api.Assertions.assertThatThrownBy(
            () -> service.list(ACTOR, Role.ADMIN, "!!not-base64!!", null, null)
