 ...
 * {@link com.ibizdrive.share.ShareExpirationJob})은 각자 {@code @ConditionalOnProperty}로 enabled 게이트.
 * 잡 빈이 하나도 등록되지 않으면 single-thread scheduler는 idle 상태로 비용 무시 가능.
 *
 ...
 *
 * <p>스케줄러는 단일 thread로 충분 (현재 잡 2개, 모두 짧은 batch). 다중 잡이 동시에 무거워지면 별도
 * {@code TaskScheduler} pool 빈 도입 검토.
 */
@Configuration
@EnableScheduling
@EnableConfigurationProperties({HardPurgeProperties.class, ShareExpirationProperties.class})
