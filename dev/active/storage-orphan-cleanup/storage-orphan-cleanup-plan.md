...
StorageOrphanCleanupJob.run()
  └→ StorageOrphanCleanupService.runDailyCleanup(maxPerRun, graceHours)
       1. liveSet = repository.streamActiveStorageKeys()  (SELECT v.storageKey FROM FileVersion v — no deleted_at filter; trash 보호)
...
### OC.3
- [ ] `FileVersionRepository.streamActiveStorageKeys()` — **모든** `file_versions.storage_key`를 lazy `Stream<UUID>`로 반환. **No filter on `files.deleted_at`** — 30일 trash 동안 file_versions 행이 보존되므로 storage도 보존되어야 한다(restore 가능). A7 hard purge가 file_versions cascade로 행 자체를 삭제하면 그 시점 이후 본 stream에서 해당 key가 빠져 다음 cron에서 orphan으로 처리된다.
- [ ] 테스트: (a) active file의 모든 version storage_key 포함, (b) soft-deleted(trash) file의 version storage_key도 포함(A7 hard purge 전까지 보호), (c) 빈 DB → empty.
- [ ] **plan revision rationale**: 이전 plan은 "`JOIN files WHERE deleted_at IS NULL`" 필터를 적시했으나, 이는 trash row의 storage를 즉시 orphan으로 분류해 restore 시 read 실패를 유발한다. 단순 `SELECT v.storageKey FROM FileVersion v` 쿼리가 안전하며 cascade 패턴과 정합(CLAUDE.md §3 원칙 9 — 문제 은폐 금지).
