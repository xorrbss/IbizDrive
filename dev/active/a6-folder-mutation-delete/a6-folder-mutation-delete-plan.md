...
**out-of-scope (별도 트랙)**:
- ~~File mutation rename/move/delete/restore~~ — **A4.8 (commit `4e720eb`)에서 단일 파일 mutation 닫힘**. 본 PR은 folder cascade가 단일 파일 FileMutationService를 호출하지 않고 batch UPDATE로 처리(아래 §A6.1 audit 정책 참조).
- Hard purge job (`purge_after` 경과 row 영구 삭제) — 배치/스케줄러 트랙(docs/04 §13).
- Frontend 휴지통 UI (docs/01 §13) — backend 계약 안정화 후 별도 마일스톤.

## 현재 상태 분석

### V5 schema (master HEAD `072cdaa` — A6 bootstrap; A4.8 file mutation + A5.0 docs는 이미 머지됨)
...
- `GlobalExceptionHandler`: `RENAME_CONFLICT`(409), `NOT_FOUND`(404), `BAD_REQUEST`(400), file `RENAME_CONFLICT` 매핑. **`RESTORE_CONFLICT` 미매핑**.
- **`FileMutationService`** (A4.8): `rename`/`move`/`delete`/`restore` 단일 파일 도메인 — folder cascade에서는 호출하지 않음(audit 정책 sec. A6.1 참조).
- **`FileRepository`** (A4.8): `lockByIdAndDeletedAtIsNull/IsNotNull`, `existsActiveByFolderAndNormalizedName{,ExcludingId}`. **batch cascade UPDATE 메서드 부재** → A6.1에서 추가.
...
**(c) 후손 파일 처리** — `FileRepository.softDeleteByFolderIds(...)` batch UPDATE 신설. **`FileMutationService.delete` 호출 안 함** — 호출 시 후손 파일마다 FILE_DELETED audit이 발행되어 1000+ row 폭증 위험. (b)와 일관성: cascade는 root FOLDER_DELETED 1건만 발행.
