...
Status: 🟢 ACTIVE — 게이트 2 통과 (M9.1 commit + dev/process 정리) → M9.2 진입 대기
---
...
- **2026-05-01 M9.1 완료** — 게이트 2 통과 (사용자 OK → commit + dev/process 정리).
...
- **Phase**: 게이트 2 통과 → M9.2 진입 대기
- **선행 완료**: M9.0 (`6e67785`) — qk.trash + 무효화 헬퍼 (afterRestore/afterPurge), 분기 (A) 확정. M9.1 — trash API client + types + softDelete 마이그.
- **다음**: M9.2 — TanStack Query hooks (useTrashList + useRestoreItem + usePurgeTrashItem) + 단위 테스트 ≥10건 GREEN.
