...
A6 closure(`fdeb610`) 다음 단계로 명시된 **hard purge 배치 잡** 트랙. `purge_after <= NOW()` 조건의 soft-deleted folders/files를 **DB에서 영구 삭제**하는 daily Spring `@Scheduled` 잡을 구현한다. S3 객체 삭제는 storage 모듈 부재로 ADR #31 deferred(orphan 잔존). 휴지통 admin endpoint(`/api/trash/*`)는 별도 트랙(A8 후보)로 분리.

## 단위 분할 — 단일 PR (A2/A3/A5/A6 패턴)

추정 5~8 commits. KISS — 단일 PR.

- **A7.0** ADR #31 신설 + docs/02 §2.5 line 37 주석(version cascade) + docs/02 §7 batch 행 신설 + docs/04 §13 row patch — **no-code**
...
**out-of-scope (별도 트랙)**:
- ~~S3 객체 삭제~~ — backend storage 모듈 0개. **ADR #31**: A7은 DB-only, S3 cleanup은 storage 모듈 도입 시점에 `orphan.detect` 잡과 함께 처리.
...
1. **DB-only purge (S3 deferred)** — ADR #31. `storageKey` 수집 후 audit `after_state.orphanStorageKeys`에 기록만 (편법 아닌 명시적 deferred — CLAUDE.md §3 원칙 9 충족).
...
1. ✅ ADR #31 신설 + docs/02 §2.5 line 37 주석 patch + docs/04 §13 row 보강
...
| 1 | A7.0 docs patch + ADR #31 commit | A7.1 진입 |
